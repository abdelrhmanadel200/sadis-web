...
function OtpForm() {
  ...
  const handleChange = (i: number, v: string) => {
    ...
    // Multi-character input (paste / SMS autofill / fast typing that the
    // browser batches) — distribute the digits across the boxes starting
    // at the current index instead of dropping all but the first.
    if (cleaned.length > 1) {
      const next = [...digits];
      let idx = i;
    ...
    next[i] = one;
    setDigits(next);
    if (one && i < 5) inputsRef.current[i + 1]?.focus();
    // Auto-submit when the 6th digit is entered
    if (one && i === 5 && next.every((d) => d && d.length === 1)) {
      verifyCode(next.join(''));
    }
  };
